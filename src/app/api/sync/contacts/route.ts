...
const contactQuerySchema = z.object({
  ...
  page: z.number().min(1).optional(),      // Page-based pagination
  cursor: z.string().optional(),           // Cursor-based pagination
  fields: z.enum(['phone', 'email', 'both']).optional().default('both'),
  sortBy: z.enum(['name', 'firstName', 'lastName', 'community']).optional().default('name'),
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { search, community, activated, includeDeceased, limit, page, cursor, fields, sortBy, sortOrder } = validation.data;
    ...
    let orderByField: any = { id: 'asc' };
    if (sortBy === 'name' || sortBy === 'lastName') {
      orderByField = { last_name: sortOrder };
    } else if (sortBy === 'firstName') {
      orderByField = { first_name: sortOrder };
    }

    const members = await prisma.fnmember.findMany({
      where: {
        ...where,
        // Only use cursor if page is not provided
        ...(cursor && !page && { id: { gt: cursor } }),
      },
      include: {
        profile: {
          select: {
    ...
    const contacts = members.map(member => {
        ...
        return {
          id: member.id,  // Add id for frontend compatibility
          memberId: member.id,
