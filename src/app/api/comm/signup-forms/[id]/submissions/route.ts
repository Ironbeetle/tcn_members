...
export async function GET(
  ...
  try {
    ...
    const mappedSubmissions = submissions.map(sub => ({
      id: sub.id,
      formId: id,
      data: sub.responses as Record<string, any>,
      submittedAt: sub.created.toISOString(),
      submittedBy: sub.fnmember ? `${sub.fnmember.first_name} ${sub.fnmember.last_name}` : undefined,
      memberEmail: sub.fnmember?.profile?.[0]?.email,
    }));
