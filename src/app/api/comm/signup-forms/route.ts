...
import { comm_FormCategory } from '@prisma/client';
import { 
  validateAuth, 
  apiSuccess, 
  apiError, 
...
import { FORM_CATEGORIES } from '@/lib/category-constants';
import { z } from 'zod';
...
const formSchema = z.object({
  ...
  category: z.enum(FORM_CATEGORIES).default('BAND_OFFICE'),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Use createdBy from request or fall back to authenticated user
    const creatorId = createdBy || authResult.user?.id;
    if (!creatorId) {
      return apiError('Creator ID is required', 400);
    }
    ...
    const form = await prisma.comm_SignUpForm.create({
      data: {
        ...
        category: category as comm_FormCategory,
    ...
    // Map comm_FormCategory to FormCategory (same values now)
