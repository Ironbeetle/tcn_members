...
import { BULLETIN_CATEGORIES, BULLETIN_CATEGORY_LABELS, type BulletinCategoryValue } from '@/lib/category-constants';
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
const categoryMeta: Record<BulletinCategoryValue, { icon: any; color: string }> = {
  CHIEF_COUNCIL: { icon: Users, color: 'blue' },
  COMMUNITY_EVENTS: { icon: Calendar, color: 'pink' },
  ECONOMIC_DEVELOPMENT: { icon: Briefcase, color: 'teal' },
  YOUTH: { icon: Users, color: 'purple' },
  LAND_TREATY: { icon: Building2, color: 'brown' },
  CULTURAL_SPIRITUAL: { icon: Heart, color: 'orange' },
  EDUCATION: { icon: GraduationCap, color: 'indigo' },
  EMPLOYMENT: { icon: Briefcase, color: 'green' },
  HEALTH_FAMILY: { icon: Heart, color: 'red' },
  COMMUNITY_INFRASTRUCTURE: { icon: Building2, color: 'gray' },
  RECREATION: { icon: Calendar, color: 'cyan' },
  COMMUNITY_MEETINGS: { icon: Users, color: 'indigo' },
  COMMUNITY_ADMIN: { icon: Bell, color: 'slate' },
  TRSC: { icon: Building2, color: 'gray' },
};
...
const categories = [
  ...
  ...BULLETIN_CATEGORIES.map((value) => ({
    ...
    icon: categoryMeta[value].icon,
    color: categoryMeta[value].color,
  })),
];
